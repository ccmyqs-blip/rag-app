...
# 国内加速：直接覆盖为清华镜像，避免 apt 拉包超时（基础镜像为 Debian Trixie）
RUN rm -f /etc/apt/sources.list.d/debian.sources 2>/dev/null; \
    echo "deb http://mirrors.tuna.tsinghua.edu.cn/debian/ trixie main non-free non-free-firmware" > /etc/apt/sources.list && \
    echo "deb http://mirrors.tuna.tsinghua.edu.cn/debian-security trixie-security main non-free non-free-firmware" >> /etc/apt/sources.list && \
    echo "deb http://mirrors.tuna.tsinghua.edu.cn/debian/ trixie-updates main non-free non-free-firmware" >> /etc/apt/sources.list
